...
pub fn create_postinstall_page(builder: &Builder) {
    ...
    let paconf_btn: gtk::Button = page_builder.object("paconf").unwrap();
    ...
    let fix_smb_btn: gtk::Button = page_builder.object("fix-samba").unwrap();

    paconf_btn.connect_clicked(on_paconf_btn_clicked);
    ...
    fix_smb_btn.connect_clicked(move |_| {
        let _ = utils::run_cmd_terminal(
            String::from("/usr/share/xerowelcome/scripts/fix_samba.sh"),
            false,
        );
    });
...
pub fn init_libadw_main_button(builder: &Builder) {
    let libadw_btn: gtk::Button = builder.object("fix-libadw-gtk4").unwrap();
